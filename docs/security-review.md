...
- **Config governance invariants** — `env: prod` requires `security: strict`. `env: prod` + `LOG_LEVEL=debug` refuses to boot. Inline `api_key:` in `providers.yaml` refused at boot. All correctly wired at `internal/config/config.go:422` and `cmd/policyd/main.go`.
